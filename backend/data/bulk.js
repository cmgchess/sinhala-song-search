...
const songsWithId = songs.map((song, index) => ({id: (index+1).toString(), ...song, releasedYear: parseInt(song.releasedYear)}));
...
async function run() {
  ...
  const body = songsWithId.flatMap(doc => [{ index: { _index: INDEX_NAME } }, doc])
